...
def maybe_parse_diff(commit_id, org, out_file, project):
	...
	try:
		...
		print("Exception parsing diff", org, project, commit_id, "--", traceback.print_exc(file=sys.stdout))
	return total
...
def parse(in_dir, out_dir):
	...
	for org in orgs_list:
		...
		for project in projects_list:
			...
			for ix, commit_id in enumerate(all_commit_ids):
				result_ids.append(maybe_parse_diff.remote(commit_id, org, out_file, project))
