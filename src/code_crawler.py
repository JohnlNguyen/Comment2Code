...
def get_commit_files(csv_file_name: str, repo_dir):
	comment_code_rows = []
	with open(csv_file_name, newline='') as csv_file:
		...
		for row in map(DataRow._make, reader):
			full_file_name, file_name = create_file_name(row.organization, row.project, row.file_changed, row.commit,
														 is_added=row.mode == ADDED_MODE)
			if row.mode == ADDED_MODE:
				get_postcommit_file(
					path=path_to_file(repo_dir, row.organization, row.project),
					commit_id=row.commit,
					file=row.file_changed,
					out_file=full_file_name.open('w'),
				)
			...
			comment, code, heuristic = extract_code(lineno, full_file_name)

			if comment and code and heuristic:
				comment_code_rows.append(
					CommentCodeRow(comment=comment, code=code, heuristic=heuristic, file=file_name,
...
def main(dir_path, repo_dir):
	...
	for csv_diff_file in diff_list:
		...
		try:
			data_rows = get_commit_files(path, repo_dir)
