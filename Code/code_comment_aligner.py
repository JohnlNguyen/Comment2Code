...
def main():
	...
	if config["training"]["input_type"] == "all":
		data = SimilarityDataReader(config["data"], config["vocabulary"], args.data, vocab_file=args.vocabulary)
	else:
	...
	elif config["training"]["model"] == "transformer_encoder":
		model = TransformerEncoderModel(config["transformer"], data.vocabulary.vocab_dim)
	train(model, data)
...
def train(model, data):
	...
	for epoch in range(config["training"]["num_epochs"]):
		...
		for batch in data.batcher(mode="training", input_type=config["training"]["input_type"]):
			...
			log.log_train(loss, metrics.get_acc(), metrics.get_bce(), total_batches)
...
def manhattan_similarity(x1, x2):
	manhattan_sim = tf.exp(-tf.reduce_sum(tf.abs(x1 - x2), axis=1, keepdims=True))
...
class TransformerModel(tf.keras.Model):
	...
	def call(self, comment_indices, comment_masks, code_indices, code_masks):
		...
		states = tf.concat([comment_states, code_states, distance, comment_states - code_states, comment_states + code_states], axis=-1)
	...
	def attention_layer(self, model_config):
		...
		self.layernorm1 = [tf.keras.layers.LayerNormalization(epsilon=1e-6) for _ in range(num_layers)]
		self.layernorm2 = [tf.keras.layers.LayerNormalization(epsilon=1e-6) for _ in range(num_layers)]
		self.ff_1 = [tf.keras.layers.Dense(model_config["ff_dim"], activation="relu") for _ in range(num_layers)]
		self.ff_2 = [tf.keras.layers.Dense(model_config["hidden_dim"]) for _ in range(num_layers)]
...
class SimilarityTransformerModel(tf.keras.Model):
	def __init__(self, model_config, token_vocab_dim):
		...
		self.dense = tf.keras.layers.Dense(model_config["hidden_dim"], activation='relu')
	...
	def call(self, b_comment_indices, b_comment_masks, a_comment_indices, a_comment_masks, b_code_indices, b_code_masks,
		...
		states = tf.stack([b_b_states, b_a_states, a_b_states, a_a_states], axis=1)  # [B, 4, hidden dim]
		...
		states = tf.reshape(states, shape=[states.shape[0], states.shape[-1] * states.shape[1]])  # [B, 4 * hidden dim]
...
class SharedTransformerModel(TransformerModel):
	...
	def call(self, b_comment_indices, b_comment_masks, a_comment_indices, a_comment_masks, code_indices, code_masks,
		...
		# prediction
		states = tf.reshape(states, shape=[states.shape[0], states.shape[-1] * states.shape[1]])  # [B, 2 * hidden dim]
		preds = self.classify(states)
		preds = tf.squeeze(preds, -1)
		return preds
...
class BaselineModel(tf.keras.layers.Layer):
	...
	def __init__(self, model_config, token_vocab_dim):
		...
		self.embed = tf.keras.layers.Embedding(token_vocab_dim, model_config["embed_dim"])
	...
	def call(self, comment_indices, comment_masks, code_indices, code_masks, leaks_indices, leaks_masks, training=True):
		...
		leaks_states = tf.keras.layers.Dense(256, activation="relu")(leaks_indices)

		states = tf.concat([code_states, comment_states, leaks_states], axis=-1)
