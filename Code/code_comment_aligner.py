...
def train(model, data):
	...
	def get_learning_rate():
		if "transformer" not in config["training"]["model"]: return tf.constant(config["training"]["lr"])
		return tf.constant(4e-4)#util.compute_transformer_learning_rate(config["transformer"]["base_lr"], config["transformer"]["hidden_dim"], config["transformer"]["warmup"], total_batches)
	...
	for epoch in range(config["training"]["num_epochs"]):
		...
		for batch in data.batcher(mode="training"):
			...
			if mbs % config["training"]["print_freq"] == 0:
				...
				print("MB: {0}, lr: {1:.4f}: samples: {2:,}, entropy: {3}, acc: {4}".format(mbs, lr, *metrics.get_stats()))
				metrics.flush()
...
class BaselineModel(tf.keras.layers.Layer):
	def __init__(self, model_config, token_vocab_dim):
		...
		self.embed = tf.Variable(random_init([token_vocab_dim, model_config["embed_dim"]]), dtype=tf.float32)
		self.rnns_fwd_code = [tf.keras.layers.GRU(model_config["embed_dim"]//2, return_sequences=True) for _ in range(2)]
		self.rnns_bwd_code = [tf.keras.layers.GRU(model_config["hidden_dim"]//2, return_sequences=True, go_backwards=True) for _ in range(2)]
		self.rnns_fwd_comment = [tf.keras.layers.GRU(model_config["embed_dim"]//2, return_sequences=True) for _ in range(2)]
		self.rnns_bwd_comment = [tf.keras.layers.GRU(model_config["hidden_dim"]//2, return_sequences=True, go_backwards=True) for _ in range(2)]
		self.classify = tf.keras.layers.Dense(1, activation="sigmoid")
	...
	def call(self, comment_indices, comment_masks, code_indices, code_masks):
		...
		def run_base(indices, masks, rnns_fwd, rnns_bwd):
			...
			for ix in range(len(rnns_fwd)):
				...
				states *= masks
			return states
		comment_states = run_base(comment_indices, comment_masks, self.rnns_fwd_comment, self.rnns_bwd_comment)
