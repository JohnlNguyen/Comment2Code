...
class VocabularyBuilder():
	...
	def __init__(self, vocab_config, file_contents=None, vocab_path=None):
	...
	def load_vocab(self, vocab_path):
		with open(vocab_path, "r", encoding="utf8") as f:	
			...
			vocab = [l[1] for l in vocab if int(l[0]) >= (0 if self.split_kind == "bpe" else self.vocab_cutoff)]
	...
	def build_vocab(self, file_contents, write=True):
		if self.split_kind == "bpe" and self.split_file:
			token_counts = self.build_bpe_full_file(file_contents)
			top_words = sorted(token_counts.items(), key=lambda i: i[1], reverse=True)
			top_words = [t[0] for t in top_words]# if t[1] >= self.vocab_cutoff]
		elif self.split_kind == "chars":
			top_words = [chr(c) for c in range(33, 127)]
			token_counts = {c:1000 for c in top_words}
		else:
			token_counts = {}
			self.w2i = None
			for file in file_contents:
				for label in re.split('\s+', file):
					subtokens = [label] if self.split_kind != "heuristic" else util.split_subtokens(label)
					for sub in subtokens:
						util.merge(token_counts, sub, 1)
			
			# Ensure some key tokens make it into the vocabulary
			if "<unk>" not in token_counts: token_counts["<unk>"] = max(self.vocab_cutoff, sum([c for c in token_counts.values() if c < self.vocab_cutoff]))
			for ix, s in enumerate(self.special_tokens):
				if s not in token_counts or self.split_kind == "bpe":
					token_counts[s] = int(1e9) + ix
			
			if self.split_kind == "bpe":
				print("Computing BPE on", len(token_counts), "tokens")
				token_counts = self.build_bpe(token_counts)
				top_words = sorted(token_counts.items(), key=lambda i: i[1], reverse=True)
				top_words = [t[0] for t in top_words]# if t[1] >= self.vocab_cutoff]
			else:
	...
	def build_bpe(self, token_counts):
		token_counts = [(list(t), c) for t, c in sorted(token_counts.items(), key=lambda i: i[1], reverse=True) if len(t) > 0]
		for ix in range(len(token_counts)): token_counts[ix][0][-1] += "#"
		bpe_pairs = {}
		count_table = {}
		loc_table = {}
		...
		for ix in range(33, 127):
			if chr(ix) not in bpe_pairs: bpe_pairs[chr(ix)] = 1
			if chr(ix)+"#" not in bpe_pairs: bpe_pairs[chr(ix)+"#"] = 1
				...
				while ix < len(token):
					if token[ix-1]+token[ix] == top_pair:
						if ix > 1: # Update counts of preceding token, if any
							...
							util.merge(loc_table, token[ix-2]+top_pair, tix, wrap_fn=lambda v: set([v]))
						if ix < len(token) - 1:
							count_table[token[ix]+token[ix+1]] -= token_count
							util.merge(count_table, top_pair+token[ix+1], token_count)
							util.merge(loc_table, top_pair+token[ix+1], tix, wrap_fn=lambda v: set([v]))
	...
	def build_bpe_full_file(self, strings):
		...
		for tix, token in enumerate(strings):
			for ix, c in enumerate(token):
				if c == '\n': c = '\\n'
				if c == '\t': c = '\\t'
				if c not in bpe_pairs: bpe_pairs[c] = 0
				bpe_pairs[c] += 1
				if ix == 0: continue
				pair = token[ix-1] + c
		...
		for step in range(self.bpe_limit):
			...
			for tix, cix in loc_table[top_pair]:
				...
				if prev_ix == None or string[prev_ix]+string[cix] != top_pair: continue # May happen if cix-1 was fused by previous step
