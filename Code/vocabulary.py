...
def main():
	...
	ap.add_argument("-m", "--mode", required=False, help="Type of tokenizer")
	args = ap.parse_args()
	config = yaml.safe_load(open("config.yml"))

	if args.mode == "subword":
		vocab = SubwordTextEncoder(config["vocabulary"],
								   file_contents_generator=itertools.chain.from_iterable(get_tokens(args.data)),
								   out_vocab_path=args.vocabulary)
	else:
...
class SubwordTextEncoder(object):
	def __init__(self, vocab_config, file_contents_generator=None, vocab_path=None, out_vocab_path='vocab'):
		if not vocab_path:
			self.encoder = tfds.features.text.SubwordTextEncoder.build_from_corpus(file_contents_generator,
																				   target_vocab_size=2 ** 13)
			self.save_vocab(out_vocab_path)
		else:
			self.encoder = self.load_vocab(vocab_path)

	def load_vocab(self, vocab_path):
		return tfds.features.text.SubwordTextEncoder.load_from_file(vocab_path)
