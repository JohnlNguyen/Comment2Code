	...
	with open(out_file, 'a', newline='') as csv_file:
		...
		for file in diff.files_changed.keys():
			for (rm_start, rm_end, add_start, add_end) in diff.files_changed[file]:
				writer.writerow([diff.org, diff.project, diff.commit_id, len(diff.files_changed), python_files_changed, file, len(diff.files_changed[file]), rm_end - rm_start + 1, add_end - add_start + 1, rm_start, rm_end, add_start, add_end])
			if not file.endswith(".py"): continue
			file_name_pre = "files-pre/" + diff.org + '#' + diff.project + '#' + diff.commit_id + '#' + file.replace("/", "__")
			file_name_post = "files-post/" + diff.org + '#' + diff.project + '#' + diff.commit_id + '#' + file.replace("/", "__")
		...
		for project in projects_list:
			try:
				...
				with open(out_file, 'w', encoding='utf8', newline='') as csv_file:
					writer = csv.writer(csv_file, delimiter=',')
					writer.writerow(['organization', 'project', 'commit', 'no_files_changed', 'no_python_files_changed', 'file', 'no_file_diffs', 'no_lines_removed', 'no_lines_added', 'line_rm_start', 'line_rm_end', 'line_add_start', 'line_add_end'])
				...
				for ix, commit_id in enumerate(all_commit_ids):
					...
					try:
						parse(Diff(org, project, commit_id), dir_path, 'output.diff', out_file)
					except Exception as e:
						print("Exception parsing diff", org, project, commit_id, "--", e)
			except Exception as e:
				print("Exception processing project", org, project, "--", e)
